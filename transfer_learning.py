...
from utils import base_network_init, preprocessed_input, base_sizes, base_output_pooling
from visualization import SaliencyHeatMap
import csv
import datetime
...
class LossHistory(Callback):
    def on_train_begin(self, logs={}):
        ...
        self.train_acc_csv = str(self.stamp) + 'train_acc.csv'
        self.train_loss_csv = str(self.stamp) + 'train_loss.csv'
        self.test_acc_csv = str(self.stamp) + 'test_acc.csv'
        with open(self.train_acc_csv, 'w')as f:
            f_csv = csv.writer(f)
            f_csv.writerow(['step', 'value'])
        with open(self.train_loss_csv, 'w')as f:
            f_csv = csv.writer(f)
            f_csv.writerow(['step', 'value'])
        with open(self.test_acc_csv, 'w')as f:
            f_csv = csv.writer(f)
            f_csv.writerow(['step', 'value'])
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        self.logs_record(len(self.accuracy['batch']), self.accuracy['batch'][-1], self.train_acc_csv)
        self.logs_record(len(self.losses['batch']), self.losses['batch'][-1], self.train_loss_csv)
    ...
    def on_epoch_end(self, batch, logs={}):
        ...
        self.logs_record(len(self.val_acc['epoch']), self.val_acc['epoch'][-1], self.test_acc_csv)

    def logs_record(self, step, value, path):
    ...
    def loss_plot(self):
        ...
        plt.figure()
        # loss
        plt.plot(iters, self.val_loss['batch'], 'k', label='val loss')
        plt.xlabel('Iters')
        plt.ylabel('Loss')
...
class DenseLearner:
    def __init__(self, layers=None, dropout=None, base='resnet50', pooling='average', objective='categorical',
                 **kwargs):
        if dropout is None:
            dropout = [0.5]
        if layers is None:
    ...
    def flow_from_directory(self, train, val, train_augs_generator=None,
                            val_augs_generator=None, epochs=10, batch_size=32, unfreeze=0, optimizer=None,
                            use_default_callbacks=True, custom_callbacks=None,
                            load_from_checkpoint=False):
        if not self._classes:
            classes = list(os.listdir(train))
            self._classes = np.unique(classes).tolist()
            self._class_to_idx = {value: index for index, value in enumerate(self._classes)}
            self._idx_to_class = {index: value for index, value in enumerate(self._classes)}
        ...
        if not train_augs_generator:
            train_datagen = ImageDataGenerator(preprocessing_function=preprocessed_input.get(self._base),
                                               ...
                                               horizontal_flip=True,
                                               fill_mode='nearest')
            train_augs_generator = train_datagen.flow_from_directory(train,
                                                                     target_size=base_sizes.get(self._base),
                                                                     batch_size=batch_size,
                                                                     classes=self._classes,
                                                                     class_mode=self._class_mode)
        ...
        if not val_augs_generator:
            val_datagen = ImageDataGenerator(preprocessing_function=preprocessed_input.get(self._base))
            val_augs_generator = val_datagen.flow_from_directory(val,
                                                                 shuffle=False,
                                                                 target_size=base_sizes.get(self._base),
                                                                 batch_size=batch_size,
                                                                 classes=self._classes,
        ...
        train_steps = len(get_all_paths(train)) // batch_size
        val_steps = len(get_all_paths(val)) // batch_size
    ...
    def _compile(self, path=None, optimizer=None, unfreeze=0):
        ...
        for dense_units, dropout_units in zip(self._layers, self._dropout):
            ...
            x = tf.keras.layers.Dropout(rate=dropout_units)(x)
        activation = 'sigmoid' if self._objective == 'binary' else 'softmax'
        output_units = 1 if self._objective == 'binary' else len(self._classes)
        output = tf.keras.layers.Dense(output_units, activation=activation)(x)

        self._compiled_model = tf.keras.Model(inputs=base.input, outputs=output)
        optimizer = optimizer if optimizer else tf.keras.optimizers.Adam()
